...
def capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite):
    ...
    def save_hdf5(dataset_path, camera_names, max_timesteps, padded_size, COMPRESS, data_dict, compressed_len):
        """在后台线程中保存 HDF5 数据"""
    ...
    # 启动后台线程
    thread = threading.Thread(target=save_hdf5, args=(
    dataset_path, camera_names, max_timesteps, padded_size, COMPRESS, data_dict, compressed_len))
    thread.start()

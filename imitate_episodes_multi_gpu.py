...
def main(args):
    ...
    if not is_eval:
        wandb.init(project="tactile_aloha", reinit=True, entity="guningquan", name=expr_name)
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=1):
    ...
    Length_multiple = 1.2 # may increase or decrease for real-world tasks
            ...
            for t in range(max_timesteps):
                ...
                if config['policy_class'] == "ACT":
                    ...
                    if temporal_ensemble:
                        ...
                        #     raw_action[0, -2:] = 0
                elif config['policy_class'] == "TactileACT":
                    if t % query_frequency == 0:
                        if vq:
                            if rollout_id == 0:
                                for _ in range(10):
                    ...
                    if t % query_frequency == 0:
                        ...
                        #     collect_base_action(all_actions, norm_episode_all_base_actions)
                        if real_robot and BASE_DELAY == 0:  # gnq added
                            all_actions = torch.cat([all_actions[:, :, :-2], all_actions[:, :, -2:]], dim=2)
                        else:
                            all_actions = torch.cat(
                                [all_actions[:, :-BASE_DELAY, :-2], all_actions[:, BASE_DELAY:, -2:]], dim=2)
                    raw_action = all_actions[:, t % query_frequency]
